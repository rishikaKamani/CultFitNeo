  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-center justify-around gap-6 py-10 px-6">
        {trustStats.map((s, i) => {
          const count = useCounter(s.value, inView, 1600 + i * 150)
          return (
            <motion.div
              key={s.label}
            ...
            >
              ...
              <div className="flex items-end gap-0.5 leading-none">
                ...
                <span className="text-2xl font-black text-[#E63946] mb-0.5">{s.suffix}</span>
              </div>
              <span className="text-gray-500 text-xs font-medium uppercase tracking-widest">{s.label}</span>
            </motion.div>
          )
        })}
      </div>
